...
export default function DuplicatesPage() {
  ...
  const duplicates = MOCK_ROMS.filter(r => r.duplicate_group_id);
      ...
      {Object.entries(groups).map(([groupId, roms]) => (
        <div key={groupId} className="pixel-border-red bg-card p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="font-retro text-xs text-muted-foreground">{roms.length} files · SHA256 match</p>
              </div>
            </div>
            <span className="font-mono text-[10px] text-muted-foreground break-all max-w-[200px]">{roms[0].hash.slice(0, 24)}...</span>
          </div>
          ...
          <div className="flex flex-wrap gap-2 pt-3 border-t border-destructive/20">
            <button
              onClick={() => toast.success('Selected files kept, others deleted')}
