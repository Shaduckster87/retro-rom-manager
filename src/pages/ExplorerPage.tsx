...
interface FolderNode {
  ...
  path: string;
  children: FolderNode[];
  files: typeof MOCK_ROMS;
}

function buildTree(roms: typeof MOCK_ROMS): FolderNode {
  ...
  // Also create console sub-folders
  const consoleFolders = new Set(
    roms.filter(r => r.storage_path.startsWith('/roms/')).map(r => {
  ...
  return (
    <div>
      <div
        ...
        className={`flex items-center gap-2 py-1.5 px-2 cursor-pointer hover:bg-muted/30 ${depth === 0 ? '' : ''}`}
      ...
      >
        ...
        <span className="font-pixel text-[7px] text-muted-foreground ml-auto">{node.files.length} files</span>
      </div>
      ...
      {expanded && (
        <>
          ...
          {node.files.map(file => (
            <div
              key={file.id}
              className="flex items-center gap-2 py-1 px-2 hover:bg-muted/20 group"
              style={{ paddingLeft: `${(depth + 1) * 20 + 8}px` }}
              draggable
            >
              <File className="w-3 h-3 text-muted-foreground" />
              <span className="font-mono text-sm text-foreground flex-1">{file.filename}</span>
              <span className="font-mono text-[10px] text-muted-foreground">{formatFileSize(file.file_size)}</span>
              <StatusBadge status={file.status} className="text-[7px]" />
              <button
                onClick={(e) => { e.stopPropagation(); toast.info(`Move ${file.filename}`); }}
...
export default function ExplorerPage() {
  const tree = buildTree(MOCK_ROMS);
